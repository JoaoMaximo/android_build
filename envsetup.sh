...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^jellaxy_") ; then
       JELLAXY_BUILD=$(echo -n $1 | sed -e 's/^jellaxy_//g')
       NAM_VARIANT=$(echo -n $1 | sed -e 's/^jellaxy_//g')
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${JELLAXY_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch jellaxy_$target-userdebug
        fi
    fi
    return $?
}
...
function installboot()
{
    ...
    then
        ...
        echo "The connected device does not appear to be $JELLAXY_BUILD, run away!"
    fi
}
